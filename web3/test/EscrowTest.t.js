...
describe("Escrow", () => {
  ...
  describe("onlyOwnerAceess", () => {
    ...
    describe("success", () => {
      ...
      it("set owner Percentage", async () => {
        transaction = await escrow.connect(deployer).setownerPercentage(20);
        result = await transaction.wait();
        const Percentage = await escrow.connect(deployer).ownerPercentage();
        expect(Percentage).to.be.equal(20);
      })
    ...
    describe("faliure", () => {
      ...
      it("handel only owner revert in set owner Percentage", async () => {
        await expect(escrow.connect(buyer).setownerPercentage(20)).to.be.reverted;
      })
    })
  ...
  describe("EscrowCreation", () => {
    ...
    describe("faliure", () => {
      ...
      it("handel invalid amount failure", async () => {
       ...
       it("handel seller and buyer both same", async () => {
        await expect(escrow.connect(buyer).dealCreation(buyer.address, "ui design", "design ui for web3 webiste", tokens(0), 7)).to.be.reverted
      })
  ...
  describe("confirmation", async () => {
    ...
    describe("success", () => {
      ...
      it("check changes in deal", async () => {
        ...
        expect(deal.status).to.be.equal(4);
      ...
      it("confirmation event", async () => {
        ...
        expect(args.title).to.be.equal("ui design");
        expect(args.description).to.be.equal("design ui for web3 webiste");
        expect(args.status).to.be.equal("Confirmation");
        expect(args.isDisputed).to.be.equal(false);
  ...
  describe("Open Dispute", async () => {
    ...
    describe("failure", () => {
      ...
      describe("amount not funded by buyer", () => {
        it("handel amount not funded faliure", async () => {
          await expect(escrow.connect(buyer).openDispute(1)).to.be.reverted;
        })
      })

      describe("amount not funded by seller", () => {
        it("handel amount not funded faliure", async () => {
          await expect(escrow.connect(seller).openDispute(1)).to.be.reverted;
        })
      })

      describe("amount funded but not deliverd product seller - buyer open dispute ", () => {
        it("handel amount funded but not deliverd product seller faliure", async () => {
          transaction = await escrow.connect(buyer).deposit(1, { value: tokens(1) });
          result = await transaction.wait()
      ...
      describe("amount funded but not deliverd product seller - seller open dispute ", () => {
        it("handel amount funded but not deliverd product seller faliure", async () => {
          transaction = await escrow.connect(buyer).deposit(1, { value: tokens(1) });
          result = await transaction.wait()
      ...
      describe("amount funded and deliverd by seller - buyer open dispute ", () => {
        it("handel amount funded and deliverd product buyer open dispute", async () => {
          transaction = await escrow.connect(buyer).deposit(1, { value: tokens(1) });
          result = await transaction.wait()
      ...
      describe("amount funded and deliverd by seller - seller open dispute ", () => {
        it("handel amount funded and deliverd product seller open dispute", async () => {
          transaction = await escrow.connect(buyer).deposit(1, { value: tokens(1) });
          result = await transaction.wait()
          ...
          await (escrow.connect(seller).openDispute(1));
        })
      })

      describe("deal is confirm then open dispute ", () => {
        it("handel deal is confirm then open dispute failure", async () => {
          transaction = await escrow.connect(buyer).deposit(1, { value: tokens(1) });
          result = await transaction.wait()
  ...
  describe("Dao Voting", async () => {
    describe("success", () => {
      ...
      it("check voting", async () => {
        ...
        expect(await escrow.connect(buyer).gethasvoted(1, user.address)).to.be.equal(true);
    ...
    describe("failure", () => {
      ...
      describe("amount funded and delivered", () => {
        ...
        it("handel second time vote same dispute", async () => {
          ...
          await escrow.connect(user).vote(1, false, tokens(0.5));

          await expect(escrow.connect(user).vote(1, false, tokens(0.5))).to.be.reverted;
        })
  ...
  describe("close Dispute", () => {
    describe("success", () => {
      ...
      beforeEach(async () => {
        ...
        dispute = await escrow.connect(buyer).getDispute(1);
  ...
  describe("unlock stake from dispute", () => {
    describe("success", () => {
      ...
      it("unlock stake event", async () => {
        const event = result.events[0];
        const args = event.args;

        expect(event.event).to.be.equal("UnlockStake");
        expect(args.disputedId).to.be.equal(1);
        expect(args.dealId).to.be.equal(1);
        expect(args.unlockstatus).to.be.equal("true");
      })
  ...
  describe("getter function tests", () => {
    it("get deal", async () => {
      const deal = await escrow.connect(buyer).getDeal(1);
      expect(deal.dealId).to.be.equal(1);
    })

    it("get deposited", async () => {
      const amount = await escrow.connect(buyer).getdeposited(buyer.address);
      expect(amount).to.be.equal(0);
    })

    it("get total Deposit As Buyer", async () => {
      const amount = await escrow.connect(buyer).totalDepositAsBuyer(buyer.address);
      expect(amount).to.be.equal(0);
    })

    it("get total As reciver", async () => {
      const amount = await escrow.connect(buyer).totalreciveAsSeller(buyer.address);
      expect(amount).to.be.equal(0);
    })

    it("get staked", async () => {
      const amount = await escrow.connect(buyer).staked(buyer.address);
      expect(amount).to.be.equal(0);
    })
    it("get havoted", async () => {
      const voted = await escrow.connect(buyer).hasvoted(1, buyer.address);
      expect(voted).to.be.equal(false);
    })
